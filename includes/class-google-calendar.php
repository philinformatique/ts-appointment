...
class TS_Appointment_Google_Calendar {
    ...
    public function create_event($appointment, $is_confirmed = null) {
        ...
        $event = array(
            'summary' => $status_prefix . $service->name . ' - ' . TS_Appointment_Email::get_client_value($appointment, 'client_name'),
            'description' => 'Client: ' . TS_Appointment_Email::get_client_value($appointment, 'client_name') . "\n" .
                           'Email: ' . TS_Appointment_Email::get_client_value($appointment, 'client_email') . "\n" .
                           'Téléphone: ' . TS_Appointment_Email::get_client_value($appointment, 'client_phone') . "\n" .
                           'Type: ' . $this->get_appointment_type_label($appointment->appointment_type) . "\n" .
                           'Notes: ' . TS_Appointment_Email::get_client_value($appointment, 'notes') . "\n" .
                           $status_line,
            'start' => array(
                'dateTime' => $start_iso,
            ...
            'transparency' => 'opaque',
            'attendees' => array(
                array('email' => TS_Appointment_Email::get_client_value($appointment, 'client_email')),
            ),
        );
        ...
        $client_address = TS_Appointment_Email::get_client_value($appointment, 'client_address');
        if ($client_address) {
        ...
        // Respect sendUpdates option: none|externalOnly|all|onConfirmation
        $sendUpdates = get_option('ts_appointment_google_send_updates', 'none');
        if ($sendUpdates === 'onConfirmation' && !$confirmed) {
            // If configured to notify only on confirmation, disable sending for unconfirmed
            $sendUpdates = 'none';
        }
    ...
    public function update_event($appointment) {
        ...
        $sendUpdates = get_option('ts_appointment_google_send_updates', 'none');
        if ($sendUpdates === 'onConfirmation' && !$is_confirmed) {
            $sendUpdates = 'none';
        }
